...
<script>
            ...
            columns: [
                ...
                { data: "nombre" },
                {
                    render: function ( data, type, row ) {
                        return '<button class="btn btn-info btn-sm" onClick="abrirDetalle(' + row.id_matriz + ')"><i class="fas fa-info"></i> Detalle</button> <button class="btn btn-primary btn-sm" onClick="abrirModal('+row.id_matriz+')"><i class="fas fa-edit"></i> Editar</button> <button class="btn btn-danger btn-sm" onClick="eliminarMatriz('+row.id_matriz+')"><i class="fas fa-times"></i> Eliminar</button>';
                    },
                    targets: 3
                }
            ],
                    ...
                    $(response.matriz.conexiones).each(function(i, v){
                        let destino = v.tipo == 'C'? "Canal "+ v.canal_destino.id_canal : v.nombre;
                            ...
                            if (v.tipo == 'C') {
                                ...
                                $("#tabla_data_matriz > tbody").append("<tr><td>"+v.id_canal_origen+"-Canal "+" - "+ tecnology.nombre+" - "+ v.canal_origen.tecnologia_operador.operador.nombre+" - "+numero_temp+ "</td><td>"+  v.numero_externo.id_numero_externo+ " - "+ v.numero_externo.nombre+ " - " + v.numero_externo.numero+"</td><td><button class='btn btn-danger btn-sm' onClick='eliminarDato(this)'><i class='fas fa-times'></i> </button></td></tr>");
                            }
                    });
                }
            });
    ...
    function abrirDetalle(id_matriz) {
        ...
        console.log(url_matrices + "/" + id_matriz);
